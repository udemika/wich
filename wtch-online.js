(function() {
  ...
  function isAllowedSource(j) {
    var n = (j.name || '').toLowerCase();
    return (n.indexOf('filmix') >= 0 && n.indexOf('2160p') >= 0) || (n.indexOf('lumex') >= 0 && n.indexOf('1080') >= 0) || (n.indexOf('eneyida') >= 0 && (n.indexOf('украинский') >= 0 || n.indexOf('український') >= 0));
  }
  function getSourceDisplayName(j) {
    var n = (j.name || '').toLowerCase();
    if (n.indexOf('eneyida') >= 0 && (n.indexOf('украинский') >= 0 || n.indexOf('український') >= 0)) return 'Eneyida \uD83C\uDDFA\uD83C\uDDE6';
    return j.name || '';
  }
  function sortSourcesWithEneyidaFirst(keys) {
    return keys.slice().sort(function(a, b) {
      if (a.indexOf('eneyida') >= 0) return -1;
      if (b.indexOf('eneyida') >= 0) return 1;
      return 0;
    });
  }
...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  function component(object) {
    ...
    this.startSource = function(json) {
      return new Promise(function(resolve, reject) {
        json = json.filter(isAllowedSource);
        json.forEach(function(j) {
          var name = balanserName(j);
          sources[name] = {
            url: j.url,
            name: getSourceDisplayName(j),
            show: typeof j.show == 'undefined' ? true : j.show
          };
        });
        filter_sources = sortSourcesWithEneyidaFirst(Lampa.Arrays.getKeys(sources));
      ...
      return new Promise(function(resolve, reject) {
        ...
        var fin = function fin(call) {
          ...
          network.silent(account(url), function(json) {
            ...
            filter_sources = [];
            sources = {};
            (json.online || []).filter(isAllowedSource).forEach(function(j) {
              var name = balanserName(j);
              sources[name] = {
                url: j.url,
                name: getSourceDisplayName(j),
  ...
  function startPlugin() {
    ...
    Lampa.Lang.add({
      ...
      lampac_no_watch_history: {
        ru: 'Немає історії перегляду',
        en: 'No browsing history',
        ua: 'Немає історії перегляду',
        zh: '没有浏览历史'
      },
      lampac_nolink: {
        ru: 'Неможливо отримати посилання',
      ...
      helper_online_file: { //
        ru: 'Утримуйте клавішу "ОК" для виклику контекстного меню',
      ...
      lampac_voice_subscribe: { //
        ru: 'Підписатися на переклад',
        uk: 'Підписатися на переклад',
        en: 'Subscribe to translation',
        zh: '订阅翻译'
      },
      lampac_voice_success: { //
        ru: 'Ви успішно підписалися',
      ...
      lampac_clear_all_marks: { //
        ru: 'Очистити всі мітки',
        uk: 'Очистити всі мітки',
        en: 'Clear all labels',
        zh: '清除所有标签'
      },
      lampac_clear_all_timecodes: { //
        ru: 'Очистити всі тайм-коди',
      ...
      lampac_balanser_dont_work: { //
        ru: 'Пошук на ({balanser}) не дав результатів',
        uk: 'Пошук на ({balanser}) не дав результатів',
        en: 'Search on ({balanser}) did not return any results',
        zh: '搜索 ({balanser}) 未返回任何结果'
      },
      lampac_balanser_timeout: { //
        ru: 'Джерело буде автоматично переключено через <span class="timeout">10</span> секунд.',
